...
void UKF::Prediction(double delta_t) {
  ...
  */
  ...
  for (int i = 0; i< n_aug_; i++)
  {
    ...
    Xsig_aug(3, i+1) = NormalizeAngle(Xsig_aug(3, i+1));
//    while(Xsig_aug(3, i+1) > M_PI) Xsig_aug(3, i+1)-=2*M_PI;
//    while(Xsig_aug(3, i+1) < -M_PI) Xsig_aug(3, i+1)+=2*M_PI;
    Xsig_aug(3, i+1+n_aug_) = NormalizeAngle(Xsig_aug(3, i+1+n_aug_));
//    while(Xsig_aug(3, i+1+n_aug_) > M_PI) Xsig_aug(3, i+1+n_aug_)-=2*M_PI;
//    while(Xsig_aug(3, i+1+n_aug_) < -M_PI) Xsig_aug(3, i+1+n_aug_)+=2*M_PI;
    Xsig_aug(4, i+1) = NormalizeAngle(Xsig_aug(4, i+1));
//    while(Xsig_aug(4, i+1) > M_PI) Xsig_aug(4, i+1)-=2*M_PI;
//    while(Xsig_aug(4, i+1) < -M_PI) Xsig_aug(4, i+1)+=2*M_PI;
    Xsig_aug(4, i+1+n_aug_) = NormalizeAngle(Xsig_aug(4, i+1+n_aug_));
//    while(Xsig_aug(4, i+1+n_aug_) > M_PI) Xsig_aug(4, i+1+n_aug_)-=2*M_PI;
//    while(Xsig_aug(4, i+1+n_aug_) < -M_PI) Xsig_aug(4, i+1+n_aug_)+=2*M_PI;
  }
...
//    while (yaw_p<-M_PI) yaw_p+=2.*M_PI;
    ...
    yawd_p = NormalizeAngle(yawd_p);
//    while (yawd_p> M_PI) yawd_p-=2.*M_PI;
//    while (yawd_p<-M_PI) yawd_p+=2.*M_PI;
...
void UKF::UpdateLidar(MeasurementPackage meas_package) {
  ...
  */
  ...
  VectorXd z_pred_2 = VectorXd(n_z);
  z_pred_2.fill(0.0);
  z_pred_2 << z_pred(0), z_pred(1);
  
  VectorXd y = meas_package.raw_measurements_ - z_pred_2;
  ...
  */
  ...
  for (int i = 0; i < 2 * n_aug_ + 1; i++) {  //2n+1 simga points
    ...
    if((p_x*p_x + p_y*p_y) < 0.000001){
      if(p_x < 0.0001 && p_x >=0){
        p_x = 0.0001;
      } else if(p_x > -0.0001 & p_x < 0) {
        p_x = -0.0001;
      }
      if(p_y < 0.0001 && p_y >=0){
        p_y = 0.0001;
      } else if(p_y > -0.0001 & p_y < 0) {
        p_y = -0.0001;
      }
    }
...
//  while(meas_package.raw_measurements_(1) < -M_PI) meas_package.raw_measurements_(1)+=2*M_PI;
  ...
  //angle normalization
  z_diff2(1) = NormalizeAngle(z_diff2(1));
...
//  while (z_diff2(1)<-M_PI) z_diff2(1)+=2.*M_PI;
  ...
  double nis = z_diff2.transpose()*S.inverse()*z_diff2;
  cout << "radar nis = " << nis << endl;
  //  cout << "radar nis(2) = " << nis(2) << endl;
}
